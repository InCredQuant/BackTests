# -*- coding: utf-8 -*-
"""
Created on Wed Oct 12 17:41:01 2022

@author: Viren@Incred
Testing AssymetricWeekly:
...
class AssymetricWeekly(FactoryBackTester):
    ...
    def basicdatainitialize(self):
        ClosePrice = self.BackTestData.SMA20.loc[:, self.ticker].dropna()
        ...
        self.UpdateDates = list(self.BackTestData.SMA20.index)#list(self.BackTestData.Close.index)#.values()
        ...
        self.CloseDiffMinSMA = numpy.subtract(self.BackTestData.Close, numpy.minimum(self.BackTestData.SMA10, self.BackTestData.SMA20))
    ...
    def UpdateSpecificStats(self):
        if numpy.isnan(self.BackTestData.SMA20.loc[self.CurrentTime, self.ticker]):
            self.order.symbol = self.ticker
            self.order.segment = Segment.EQ
            self.order.quantity = 1
        ...
        elif self.CloseDiffMinSMA.loc[self.CurrentTime, self.ticker] < 0:
...
if __name__=='__main__':
    ...
    f.close()
    basePath = dirPath+'BacktestsResults/DailyTradingSignals/AssymetricWeekly_'
    ...
    for ticker in mydata.Close.columns:
        try:
            a = AssymetricWeekly(mydata, ticker)
            a.run()
            print(ticker)
