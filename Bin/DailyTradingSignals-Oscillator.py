# -*- coding: utf-8 -*-
"""
Created on Thu Oct 13 09:05:54 2022
...
class Oscillator(FactoryBackTester):
    ...
    def basicdatainitialize(self):
        ClosePrice = self.BackTestData.ATR24.loc[:, self.ticker].dropna()
        ...
        self.UpdateDates = list(self.BackTestData.ATR24.index)#list(self.BackTestData.Close.index)#.values()
        ...
        self.Ratio = numpy.subtract(self.BackTestData.Close, self.BackTestData.SMA24)/self.BackTestData.ATR24
    ...
    def UpdateSpecificStats(self):
        if numpy.isnan(self.BackTestData.ATR24.loc[self.CurrentTime, self.ticker]):
            self.order.symbol = self.ticker
            self.order.segment = Segment.EQ
            self.order.quantity = 1
        ...
        elif self.Ratio.loc[self.CurrentTime, self.ticker] < -1:
            self.Position.loc[self.CurrentTime, self.ticker] = -1
        elif self.Ratio.loc[self.CurrentTime, self.ticker] > 1:
            self.Position.loc[self.CurrentTime, self.ticker] = 1
        elif (self.Position.loc[self.LastTime, self.ticker] == 1 and self.Ratio.loc[self.CurrentTime, self.ticker] < 0) or (self.Position.loc[self.LastTime, self.ticker] == -1 and self.Ratio.loc[self.CurrentTime, self.ticker] > 0):
            self.Position.loc[self.CurrentTime, self.ticker] = 0
        else:
...
if __name__=='__main__':
    ...
    f.close()
    basePath = dirPath+'BacktestsResults/DailyTradingSignals/Oscillator_'
