# -*- coding: utf-8 -*-
"""
Created on Wed Oct 12 17:28:26 2022

@author: Viren@Incred
Testing RSI(14)_50:
    if RSI(14) > 50 then Long if RSI(2)< 50 then Short   
...
class Model_RSI50(FactoryBackTester):
    ...
    def basicdatainitialize(self):
        ClosePrice = self.BackTestData.RSI2.loc[:, self.ticker].dropna()
        ...
        self.UpdateDates = list(self.BackTestData.RSI2.index)#list(self.BackTestData.Close.index)#.values()
    ...
    def UpdateSpecificStats(self):
        if numpy.isnan(self.BackTestData.RSI14.loc[self.CurrentTime, self.ticker]):
            self.order.symbol = self.ticker
            self.order.segment = Segment.EQ
            self.order.quantity = 1
        ...
        elif self.BackTestData.RSI14.loc[self.CurrentTime, self.ticker] > 50:
...
if __name__=='__main__':
    ...
    f.close()
    basePath = dirPath+'BacktestsResults/DailyTradingSignals/RSI50_'
