# -*- coding: utf-8 -*-
"""
Created on Thu Oct 13 09:51:32 2022
...
class Vortex(FactoryBackTester):
    ...
    def basicdatainitialize(self):
        ClosePrice = self.BackTestData.PVI.loc[:, self.ticker].dropna()
        ...
        self.UpdateDates = list(self.BackTestData.PVI.index)#list(self.BackTestData.Close.index)#.values()
        ...
        self.Ratio = numpy.subtract(self.BackTestData.PVI, self.BackTestData.NVI)
    ...
    def UpdateSpecificStats(self):
        if numpy.isnan(self.BackTestData.PVI.loc[self.CurrentTime, self.ticker]):
            self.order.symbol = self.ticker
            self.order.segment = Segment.EQ
            self.order.quantity = 1
        ...
        elif self.Ratio.loc[self.CurrentTime, self.ticker] > 0:
...
if __name__=='__main__':
    ...
    f.close()
    basePath = dirPath+'BacktestsResults/DailyTradingSignals/Vortex_'
