# -*- coding: utf-8 -*-
"""
Created on Sat Apr  8 20:36:44 2023

@author: Viren@InCred

Nifty/Bank Nifty Near Wk Strangle Short/Next Wk Long Backtest
"""
...
class Octopus(FactoryBackTester):
    ...
    def StopLossHandler(self):
        pass
        #self.StopLoss_Advanced(byGroup = True, StopLossLimit = -0.10)
        #self.UpdateOrderBook(strategyID = 'BankNifty_CurWkShortStraddle_NxtWkLong10pct_SL', options = 'y')
        
    def UpdateSpecificStats(self):
        if self.CurrentTime < self.EndTime:
            longTickers = self.BackTestData.TradingDates[self.CurrentTime.date()]['Long'] 
            shortTickers = self.BackTestData.TradingDates[self.CurrentTime.date()]['Short']
            allTickers = set.union(set(longTickers), set(shortTickers))
            #allTickers = list(set.intersection(set(allTickers), set(self.Close.columns)))
            self.CurTickersWt = pandas.DataFrame([-1 if it in shortTickers else 1 for it in allTickers], index = allTickers, columns = ['Quantity'])
    ...
    def updateCapAllocation(self):
        ...
        self.UpdateOrderBook(strategyID = 'BankNifty_CurWkShortStraddle_NxtWkLong10pct', options = 'y')
...
if __name__=='__main__':
    ...
    if os.path.exists('Z:/Pickles/'):
        pickleFile = 'Z:/Pickles/BankNiftyCalendarSpread_StraddleCurWk_10pctNextWk09Apr-2023.pkl'
    else:
        pickleFile = 'G:/Shared drives/BackTests/Pickles/BankNiftyCalendarSpread_StraddleCurWk_10pctNextWk09Apr-2023.pkl'
    ...
    basePath = 'G:/Shared drives/BackTests/BackTestsResults/CalendarSpread/BankNifty/'
    if not os.path.exists(basePath):
        path = Path(basePath)
        path.mkdir(parents=True)
        #os.mkdirs(basePath)       
    backtestName = 'BankNifty_CurWkShortStraddle_NxtWkLong10pct_'
