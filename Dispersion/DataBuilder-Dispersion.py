...
PriceData = GetNSEBhavCopyDatabyTicker(conn = bhavconn, tickers = mydata.Tickers, fieldName = 'Close')
...
if os.path.exists('Z:/Pickles/'):
    f = open('Z:/Pickles/NiftyDispersionData'+ datetime.datetime.today().date().strftime('%d%b-%Y') +'.pkl', 'wb')
else:
    f = open('G:/Shared drives/BackTests/Pickles/NiftyDispersionData'+ datetime.datetime.today().date().strftime('%d%b-%Y') +'.pkl', 'wb')
