...
DataStartDate = datetime.date(2016, 5, 31)
...
if os.path.exists('Z:/Pickles/'):
    f = open('Z:/Pickles/BankNiftyIronCondorData'+ datetime.datetime.today().date().strftime('%d%b-%Y') +'.pkl', 'wb')
else:
    f = open('G:/Shared drives/BackTests/Pickles/BankNiftyIronCondorData'+ datetime.datetime.today().date().strftime('%d%b-%Y') +'.pkl', 'wb')
