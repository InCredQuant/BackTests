...
class PriceMomentum(FactoryBackTester):
    def __init__(self,data, indexName, modelName, rebalType = 'm', stocksCount = [30, 50]):
        ...
        self.TransactionCostRate = 0.001# 10 bps on each transaction
    ...
    def basicdatainitialize(self):           
        ...
        self.MovingAverge = MyTechnicalLib.MovingAverage(self.Close, 200)
    ...
    def UpdateSpecificStats(self):
        ...
        elif self.ModelName == 'TimeSeriesMom':
            aboveDMA = self.Close.loc[self.CurrentTime, namesinIndex] - self.MovingAverge.loc[self.CurrentTime, namesinIndex]
            aboveDMA = aboveDMA[aboveDMA>0].dropna()
            #if len(aboveDMA) ==0:
            #    pdb.set_trace()
            self.FactorRanks = aboveDMA
    ...
    def updateCapAllocation(self):
        if len(self.FactorRanks) > 0:
            ...
            if self.ModelName in ['MonthlyMACD', 'TimeSeriesMom']:
...
if __name__=='__main__':
    ...
    rebalType = 'm'#'var' #'w', 'm', 'q'
    
    Category = rebalType.capitalize()#+'Rebal_WC'
    ...
    #ModelNum = {'TimeSeriesMom' :[2, 3, 4]}#{'1YrPriceMomVolAdj': [2, 3], '6MPriceLowVol': [2], '6MPriceMom' : [3, 4]}
    ModelNum = {'6MPriceLowVol': [2, 3], '1YrPriceMomVolAdj': [2, 3], '6MPriceMom' : [3, 4], '1YrPriceMom' : [3, 4]}
    #ModelNum = {'MonthlyMACD': [2, 3, 4, 6], 'Near52Wkhigh_NrTop15Pct': [2, 3, 4, 6], 'DrawDownAvgLow': [2, 3, 4, 6], }#'3MPriceMom' : [2, 3, 4, 6], '1YrPriceMomVolAdj': [2, 3, 4, 6], '6MPriceMom' : [2, 3, 4, 6], '1YrPriceLowVol' : [2, 3, 4, 6], '6MPriceLowVol': [2, 3, 4, 6]}
