...
class PriceMomentum(FactoryBackTester):
    ...
    def basicdatainitialize(self):           
        self.CurrentTime = min(list(self.BackTestData.IndexCompDict[self.IndexName].keys())) + datetime.timedelta(1)#pandas.to_datetime('2022-12-27')
    ...
    def UpdateSpecificStats(self):
        ...
        #turnover = self.MCapAvg.loc[self.CurrentTime, namesinIndex]
        #turnover = turnover/turnover.sum()
        #turnover = turnover.dropna().sort_values()
        #turnover = turnover.cumsum()
        #turnover = turnover[turnover>0.10]
        #namesinIndex = turnover.index
        ...
        #topNames = self.FactorRanks.iloc[-30:].index
        
        #macd = self.MACDDiff.loc[:self.CurrentTime + datetime.timedelta(4), namesinIndex].iloc[-1].dropna()
        #macd = macd[macd>0].dropna()
        #self.FactorRanks = self.FactorRanks.loc[set.intersection(set(topNames), set(macd.index))].dropna()
        
        #PriceMomEx1M = self.Close.pct_change(231).loc[:self.CurrentTime -datetime.timedelta(30), namesinIndex].iloc[-1].dropna()
        #PriceMomEx1M = PriceMomEx1M.rank(ascending = True)/len(PriceMomEx1M)
        
        #persistence = self.BackTestData.Close.resample('w').last().pct_change().loc[self.CurrentTime -  datetime.timedelta(365):self.CurrentTime, namesinIndex]
        #persistence = (persistence.rank(axis =1)/persistence.shape[1]).sum()
        #persistence = persistence.rank(ascending =  True)/len(persistence)
        #skewness = self.Skewness.loc[self.CurrentTime, namesinIndex].dropna()
        #skewness = skewness.abs().rank(ascending = False)/len(skewness)
...
if __name__=='__main__':
    ...
    #f = open(picklePath+ '20240621.pkl', 'rb')#datetime.datetime.today().date().strftime('%Y%m%d') +'.pkl', 'rb')
    ...
    basePath = os.path.join('G:/Shared drives/QuantFunds/EquityPlus/Backtests/Momentum/ThirtyFiftyStocks/', Category + datetime.datetime.now().date().strftime('_%d%b%Y/'))
    if not os.path.exists(basePath):
        os.makedirs(basePath)
    
    indices = {1 : 'NIFTY', 2: 'BSE100', 3: 'BSE200', 4: 'BSE500', 5: 'SPBSAIP'}
    numStocks = {'NIFTY': [15, 20], 'BSE100': [20, 30], 'BSE200': [30, 50], 'BSE500': [30, 50], 'SPBSAIP': [20, 30]}
    ...
    ModelNum = {'1YrPriceMomVolAdj': [2, 3], '6MPriceLowVol': [2, 3], '6MPriceMom' : [3, 4]}
