# -*- coding: utf-8 -*-
"""
Created on Fri Jan  3 19:00:42 2025
...
class PriceToBook(BackTester):    
    ...
    def UpdateSpecificStats(self):
        ...
        priceToBook = self.BackTestData.PB.loc[:self.CurrentTime, : ].iloc[-1].dropna()
        priceToBook = priceToBook.loc[priceToBook.index.isin(indexConstituents)]
        priceToBook = priceToBook.sort_values()
        
        pbRanks = priceToBook.rank(ascending = True)# Lower Price to Book is given Lowest Rank (Best- 1 is assumed Best)        
        self.FactorRanks = pbRanks[(self.Bucket-1)*int(0.2*len(pbRanks)):self.Bucket*int(0.2*len(pbRanks))]
...
if __name__=='__main__':
    ...
    for bucket in range(1, 6):
        model = PriceToBook(mydata, Bucket = bucket)
        model.run()
        
        model.ResultFrameWithIndex()
        backtestName = 'PB-'+ str(bucket)
