...
#TRADE_REGISTER = 'AllTrades_TradesRegister2024-12-26.xlsx'
#TRADE_REGISTER = 'AllTrades_TradesRegister2025-03-26.xlsx'
TRADE_REGISTER = 'AllTrades_TradesRegister2025-06-25.xlsx'
...
class DailyTradingSignals(FactoryBackTester):
    ...
    def basicdatainitialize(self):  
        ...
        self.CurrentTime = pandas.to_datetime('2024-09-26')#('2023-03-29')#pandas.to_datetime('2022-12-29')#('2022-08-31')# '2013-01-07'
        ...
        self.ModelLookBack = {'RSIwith50SMATrend' : 1, 'RSI50SMA' :2, 'Series3' : 2, '20WMA_MACD' : 2, 'BodyOutSideBand' : 2, 'ROCMA' : 2, 'RegressionCrossOver' : 1, 'Vortex' : 3, 'Oscillator' : 4, 'RSI50': 2, 'AssymetricWeekly' : 5, 'Seasoning' : 3, 'AssymetricDaily' : 2}# lookback period in Number of Quarters
        # Previous to 25 Sep 2024, we were running seasonality with 2 Quarters look back period, but now shifting to 3 quarters lookback period
        self.MODELSELECTOR = {}        
        try:
    ...
    def UpdateSpecificStats(self):
        if hasattr(self, 'TRADE_REGISTER') and str(self.CurrentTime.date()) in self.TempExpiryDates and self.CurrentTime.month in [3, 6, 9, 12]:#self.BackTestData.ExpiryDates
            ...
            PastTrades = PastTrades[[i in LiqAllstocks for i in PastTrades['SYMBOL']]]
            ...
            for grp in gg.groups.keys():
                ...
                statsSymbolDF = statsSymbolDF[statsSymbolDF['Total Trades']>= 6]#to capture significant counts, we take those models where atleast there are 3 trades per quarter
            ...
            tempProfitFactor = tempProfitFactor[tempProfitFactor['Profit Factor']>=2.1]
            ...
            tempProfitFactor = tempProfitFactor.iloc[:20]# Selecting top 25 for Seasonality, remaining will be selected from other models
            ...
            self.HistProfitFactor = self.HistProfitFactor[self.HistProfitFactor['Profit Factor']>= 2.1]
