# -*- coding: utf-8 -*-
"""
Created on Mon Mar 18 16:58:09 2024

@author: Viren@InCred
Index Constituents Covered Call 
This Backtest is using Daily Data from BhavCopy:
    1. taking Inded Spot and future prices, 
    2. Constitunets Spot Ptices
    3. Selecting the Top 10 Names based on 3 yrs Low Volatiltiy and 1 Yr Price Momemntum
    4. Then writing the ATM calls for theses Stocks Options and 
    5. One verison using put for hedge  and other paritally hedging and another one no hedge.
"""
...
class IndexConstituentsCoveredCalls(FactoryBackTester):
    def __init__(self, data, indexName, expiryDate, backtestName):
    ...
    def UpdateSpecificStats(self):
        try:
            ...
            indexChange = abs(indexChange) - 2.0*self.RollAt#(0.04/10)
        except:
            indexChange = [-1]
        if self.CurrentTime <= self.EndTime and self.RollsCount <= 5 and self.CurrentTime.time() >= self.PosStartTime and self.CurrentTime.time() <= self.PosEndTime and (indexChange[0] >= 0 or self.LastPosChangeTime == self.CurrentTime or self.CurrentTime.time() == self.PosStartTime):
            ...
            strike1 = int(numpy.round(atm*(1+self.RollAt)/100)*100)
            strike2  = int(numpy.round(atm*(1-self.RollAt)/100)*100)      
...
if __name__=='__main__':
    ...
    INDEXNAME = 'NIFTY'
    slippage = '1pct'
    strategy = f'NiftyIndexConstituents_LowVol_HighMomentum_{slippage}_Slippage'
    
    threads = 1 # How many thread are running  
    #basePath = f'{basePathDir}/Data/{INDEXNAME}_FUT_OPT_DATA/'
    
    mydata.indexcomponents = GetComponentsForIndexForDateRange(priceconn, DataStartDate, datetime.datetime.today(), INDEXNAME + ' INDEX')
    allstocks = []
    ...
    mydata.indexprice = GetDataForIndicesFromBloomDB(priceconn, [INDEXNAME + ' INDEX'], 'PX_LAST')
    ...
    for rollAt in rollList:        
        ...
        backtestName = f'{INDEXNAME.title()}{strategy}_Pt{str(int(rollAt*10000))}pct'
        ...
        def _backtest_chunk(chunk):
            ...
            for iD  in chunk:
                ...
                priceData.indexpriceSpot = pandas.DataFrame(mydata.Index.Spot.close)
                ...
                lowerStrike = 0.95*priceData.indexprice.min()[0]-200
                upperStrike = 1.05*priceData.indexprice.max()[0]+200
                ...
                model = RollingStrangle(priceData, indexName = INDEXNAME, posStartTime = posStartTime, posEndTime = posEndTime, expiryDate = expiry, rollAt = rollAt, backtestName = backtestName)
                model.run()               
