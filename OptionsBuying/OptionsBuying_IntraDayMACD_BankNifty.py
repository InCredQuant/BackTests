# -*- coding: utf-8 -*-
"""
Created on Wed Feb 15 11:05:46 2023

@author: Viren@Incred
Use: To backtest, bank Nifty Options Buying Based on MACD Signal, backtestign on 5 Mins Data
"""
...
class MACDOptionsTradingSignals(FactoryBackTester):
    ...
    def basicdatainitialize(self):
        self.CurrentTime = pandas.to_datetime('2016-06-03 09:15:00')#pandas.to_datetime('2020-04-30 09:15:00')#pandas.to_datetime('2019-03-29 09:15:00')#self.Close.index[1]#pandas.to_datetime('2022-12-01 09:15:00')#('2022-08-31')# '2013-01-07'
    ...
    def updateCapAllocation(self):
        ...
        self.UpdateOrderBook(strategyID = 'BankNiftyHourlyMACD', options = 'y')
        

if __name__=='__main__':
    import pickle
    pickleBasePath = 'G:/Shared drives/BackTests/Pickles/BankNiftyOptions/'
    pickleFile = os.path.join(pickleBasePath, 'BANKNIFTY_5Mins_1otm_May16_Dec22.pkl')    
    if os.path.exists(pickleFile):
        f = open(pickleFile, 'rb')
        mydata = pickle.load(f)
        f.close()
    else:
        os.mkdir(pickleBasePath)
        print('Data Error! Working on Data.')
        mydata = BuildOptionsData(ticker = 'BANKNIFTY', reSampleTime = 10, startDate = datetime.date(2018, 4, 27))
        #BuildOptionsData(ticker = 'NIFTY', pointsDiff = 100, otmPoints = 1, reSampleTime = 5, startDate = datetime.date(2019, 3, 21), endDate = datetime.date(2022, 12, 31), indexFields = ['Open', 'High', 'Low', 'Close'], SecFields = ['High', 'Low', 'Close'])
    ...
    SignalTime = 60# mins
    slList = [-0.25, -0.35, -0.5]
    targetList = [2.0, 2.5, 5.0]
    for sl in slList:
        for target in targetList:
            ...
            basePath = 'G:/Shared drives/BackTests/BackTestsResults/OptionsBuying/BankNiftyHourlyMACD/'
