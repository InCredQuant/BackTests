# -*- coding: utf-8 -*-
"""
Created on Mon Nov 21 12:13:30 2022

@author: Viren@Incred
Selecting Top 4 Sectors:
    3 based on 6Months Price Momentum, 6 Months Price Volatiltiy and 3 Months Price Momentum
    1 based on in top50% of the above momentum and highest correction in last 2 weeks
    
In the similar fashion, for each sector Index top 4 Stocks are slected, top momenutm from the top momenutm and lowest Momentum from the 
Highest Correction, based n last 2 Weeks Price Data
...
class SectorRotationModel(FactoryBackTester):
    ...
    def basicdatainitialize(self):
        ...
        self.DMA20 = MyTechnicalLib.MovingAverage(self.Close, 20)
        self.DMA50 = MyTechnicalLib.MovingAverage(self.Close, 50)
        self.DMA200 = MyTechnicalLib.MovingAverage(self.Close, 200)
        self.Sectors = {}
    ...
    def GetCrossMomRanks(self, ClosePrice, stocksList):
        ...
        self.TwoWkReversal = TwoWkMom.loc[TwoWkMom.index.isin(ReversalNames)].sort_values()
    ...
    def UpdateSpecificStats(self):        
        ...
        #Select Top 3 based on 6Months Highest Price Momentum and 1 based on 6Months Highest but 2Wk Lowest Price mom
        ...
        for iSector in MomSector:
            indexInclusion = self.BackTestData.IndexInclusionFactor[iSector].loc[:self.CurrentTime].iloc[-1].dropna()
            stocksList = set.intersection(set(indexInclusion.index), set(self.Close.columns))
            self.Close.loc[self.CurrentTime, stocksList]/self.DMA50.loc[self.CurrentTime, stocksList]            
            pdb.set_trace()
        #revSector, momSector <- these are the selected sectors for further selecting the stocks 
        ##################### Sector Selection Done, Stock Selection Starting #########################
        '''
        self.FactorStocks = []
        for iSector in RevSector:
            indexInclusion = self.BackTestData.IndexInclusionFactor[iSector].loc[:self.CurrentTime].iloc[-1].dropna()
            stocksList = set.intersection(set(indexInclusion.index), set(self.Close.columns))
            self.GetCrossMomRanks(self.Close, stocksList)
            self.FactorStocks.extend(list(self.TwoWkReversal[-4:].index))#TopCombined6MRank #LowTwoWkMom
        
        for iSector in MomSector:
            indexInclusion = self.BackTestData.IndexInclusionFactor[iSector].loc[:self.CurrentTime].iloc[-1].dropna()
            stocksList = set.intersection(set(indexInclusion.index), set(self.Close.columns))
            self.GetCrossMomRanks(self.Close, stocksList)
            momRanks = self.TopCombinedMomRank#self.TopCombined6MRank
            momRanks = momRanks.loc[~momRanks.isin(self.FactorStocks)]
            momRanks = momRanks.sort_values()
            self.FactorStocks.extend(list(momRanks.iloc[-4:].index))
        self.Sectors[self.CurrentTime] = set.union(set(MomSector), set(RevSector))
        '''
    ...
    def updateCapAllocation(self):
        ...
        if len(self.FactorStocks) >0 :
            ...
            StocksDF = self.FactorStocks#.iloc[-4:]
        ...
        print(self.CurrentTime.date(), len(self.FactorStocks), self.Sectors[self.CurrentTime], sep = ' ')
...
if __name__=='__main__':
    ...
    f.close()
    basePath = dirPath+'BacktestsResults/SectorRotation/Stocks/'
    
    a = SectorRotationModel(mydata, Top = True)
    a.run()
    a.ResultFrameWithIndex()
    backtestName = 'Model_Mom&Mom-Hedged'
