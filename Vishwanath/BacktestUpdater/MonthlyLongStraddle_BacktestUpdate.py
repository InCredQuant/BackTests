...
def monthly_long_straddle_backtest(optdf,eqdf,expirydf,updated_asof):
    ...
    atmdf_entry,atmdf_exit = getexitdf(mergeddf.copy(),atmdf_entry,'Long')
    ...
    os.makedirs(rf"C:\Vishwanath\PythonCodes\Strategy\MonthlyBuying\{index}\Tradebook\\Updated_{updated_asof}",exist_ok=True)
    tradebook.to_csv(rf"C:\Vishwanath\PythonCodes\Strategy\MonthlyBuying\{index}\Tradebook\\Updated_{updated_asof}\\{index.capitalize()}_LongStraddle_Monthly.csv",index=False)
    return tradebook
...
def nav_calculation(tradebook):
    ...
    def calculate_nav():
        ...
        output_path = os.path.join(base_dir,f"{index.capitalize()}_LongStraddle_{timeframe.capitalize()}_NAV.xlsx")
...
if __name__ == '__main__':
    ...
    print(f"Running Backtest for {index.capitalize()} Monthly Long Straddle")
    tradebook = monthly_long_straddle_backtest(optdf,eqdf,expirydf,updated_asof)
    print("BACKTEST UPDATED!!")
    print(f"GENERATING NAVs for {index} MONTHLY LONG STRADDLE STRATEGY")
    nav_calculation(tradebook.copy())
    print(f"CALCULATING DELTA FOR {index} MONTHLY LONG STRADDLE STRATEGY")
    deltaCalculator()
    print("COMPLETED!!")
