...
def get_last_updated_date(bip=40):
    navpath = rf"C:\Vishwanath\PythonCodes\Strategy\RollingStrangle\Outputs\{index}\NAVs\Updated_{lastupdated_asof}\\{bip}bips"
...
def strikeselector_reentry(df,bips): ## Choosing the strike for reentry based on bips
    df['EQ_Tracker'] = np.where(df['Call_Or_Put'] == 'CE',df['Close_EQ'] + (df['Close_EQ'] * (bips/10000)),df['Close_EQ'] - (df['Close_EQ'] * (bips/10000)))
    df['Difference'] = df['StrikePrice'] - df['EQ_Tracker']
    df['Difference'] = df['Difference'].abs()
    df = df.dropna(subset = ['Close_EQ'])
    result_df = df.loc[df.groupby(['Date', 'Call_Or_Put'])['Difference'].idxmin()]
    return result_df

def strikeselector(df,strategy_starttime,bips): ## As we are defining a particular start time for each system, we filter out the dataframe based on time and choose our strike based on bips
...
def rollingstrangle_signal_generator(df,strikeselectordf,entrytime,exittime,bips):
    ...
    data['BuySellFlag'] = np.where((data['CumTimeCount'] == 0), 'Sell', '') ## The first bar (first time index) is when we take a position (We go short)
    data['BuySellFlag'] = np.where((data['CumTimeCount'] == data.groupby(['Date', 'Ticker'])['CumTimeCount'].transform('max')), 'Buy', data['BuySellFlag']) ## The last bar (End of Day) is where we square off the position
    ...
    data['Stoploss'] = np.where(abs(((data['Close_EQ'] / data['PrevClose_EQ']) - 1) * 100) >= bips/100,'Triggered','') ## If the underlying has moved by a said percentage (as defined by the bips), SL gets triggered.
    data['StoplossPrice'] = np.where(data['Stoploss'] == 'Triggered', data['Close'], '')
    data['stoploss_rn'] = data.groupby(['Date', 'Ticker', 'Stoploss'])['Time'].rank().astype(int) ## Ranking the dataframe by date, ticker and stoploss to get the first instance where our entry, exit got triggered.
...
def rollingstrangle_signal_generator_reentry(data,exittime,bips):    
    data = data.reset_index().sort_values("Time")
    df = data[data["Time"] == data["Reentry_Time"]]
    strikeselectordf = strikeselector_reentry(df,bips)
    ...
    # merged_data['PrevClose_EQ'] = merged_data.groupby(['Date','Call_Or_Put'])['Close_EQ'].shift(1)
...
def reentry(signalsdf,testdf,reentry_count,entrytime,exittime,bips):
    dummy_date = datetime(2016, 5, 27)  # You can choose any date here
    ...
    for i in range(0,reentry_count):
        ...
        if len(reentry_merged_data) != 0:
            reentry_signals_df = rollingstrangle_signal_generator_reentry(reentry_merged_data, strategy_endtime,40)
...
def backtest(strategy_starttime,strategy_endtime,endtime,bips):
    if not os.path.exists(rf"C:\Vishwanath\PythonCodes\Strategy\RollingStrangle\Outputs\{index}\Tradesheet\\Updated_{updated_asof}\\"):
        os.makedirs(rf"C:\Vishwanath\PythonCodes\Strategy\RollingStrangle\Outputs\{index}\Tradesheet\\Updated_{updated_asof}\\")
    
    folderpath = rf"C:\Vishwanath\PythonCodes\Strategy\RollingStrangle\Outputs\\{index}\Tradesheet\\Updated_{updated_asof}\\"
    ...
    while iteration_starttime <= iteration_endtime: ## to run the backtest from strategy defined starttime to endtime
        ...
        for bip in bips:
            ...
            ssdf = strikeselector(df.copy(),iteration_starttime.strftime("%H:%M:%S"),bip)
            signalsdf = rollingstrangle_signal_generator(df.copy(),ssdf,iteration_starttime_time,endtime,bip)
            signalsdf = reentry(signalsdf, df.copy(), reentry_count, iteration_starttime_time, endtime,bip)
            tradebook = tradebook_generator(signalsdf.copy())
...
print(f"Running {index.capitalize()} Rolling Strangle Backtest..")
backtest(strategy_starttime,strategy_endtime,endtime,bips)
print("BACKTEST UPDATED!!")
print("GENERATING NAVs for ROLLING STRANGLE STRATEGY")

def generate_equitycurve(index,bip,start_time):
    column_name = f"NAV_{start_time.strftime('%H%M')}"
    last_nav_file_path = rf"C:\Vishwanath\PythonCodes\Strategy\RollingStrangle\Outputs\{index}\NAVs\\Updated_{lastupdated_asof}\{bip}bips\{index}_{start_time.strftime('%H%M')}_NAV.xlsx"
    lastnavfile = pd.read_excel(last_nav_file_path)
    filepath = fr"C:\Vishwanath\PythonCodes\Strategy\RollingStrangle\\Outputs\\{index}\\Tradesheet\\Updated_{updated_asof}\{bip}bips\{index}_Rolling{bip}bips_{start_time.strftime('%H%M')}.csv"
    ...
    if not os.path.exists(rf"C:\Vishwanath\PythonCodes\Strategy\RollingStrangle\Outputs\{index}\NAVs\Updated_{updated_asof}\\{bip}bips\\"):
        os.mkdir(rf"C:\Vishwanath\PythonCodes\Strategy\RollingStrangle\Outputs\{index}\NAVs\Updated_{updated_asof}\\{bip}bips\\")
    if not os.path.exists(rf"C:\Vishwanath\PythonCodes\Strategy\RollingStrangle\Outputs\{index}\NAVs\Updated_{updated_asof}\\{bip}bips\\{index}_{start_time.strftime('%H%M')}_NAV.xlsx"):
        equitycurve_unique[['Date',column_name]].to_excel(rf"C:\Vishwanath\PythonCodes\Strategy\RollingStrangle\Outputs\{index}\NAVs\Updated_{updated_asof}\\{bip}bips\\{index}_{start_time.strftime('%H%M')}_NAV.xlsx")
    
    return equitycurve_unique
    
def get_daily_nav(index):
    if not os.path.exists(rf"C:\Vishwanath\PythonCodes\Strategy\RollingStrangle\Outputs\{index}\NAVs\Updated_{updated_asof}\\"):
        os.mkdir(rf"C:\Vishwanath\PythonCodes\Strategy\RollingStrangle\Outputs\{index}\NAVs\Updated_{updated_asof}\\")
    ...
    for bip in bips:
        ...
        if not os.path.exists(rf"C:\Vishwanath\PythonCodes\Strategy\RollingStrangle\Outputs\{index}\NAVs\Combined\Updated_{updated_asof}\\"):
            os.mkdir(rf"C:\Vishwanath\PythonCodes\Strategy\RollingStrangle\Outputs\{index}\NAVs\Combined\Updated_{updated_asof}\\")
        final_NAVs.to_excel(rf"C:\Vishwanath\PythonCodes\Strategy\RollingStrangle\Outputs\{index}\NAVs\Combined\Updated_{updated_asof}\{index}RollingStrangle_{bip}bips_CombinedNAV_{updated_asof}.xlsx")
