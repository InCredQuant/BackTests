...
index = 'NIFTY'
instrument = 'OPTIDX'
strategy = 'LongStraddle' if timeframe == 'MONTHLY' else 'ShortStraddle'
type = 'Short'
...
def getexitdf(df,atmdf_entry,tradetype):
    if tradetype == 'Long':
        atmdf_exit = pd.merge(df,atmdf_entry[['Ticker','EXPIRY_DT']],left_on=['Ticker','Date'],right_on=['Ticker','EXPIRY_DT'],how='inner',suffixes=['','_tracker'])
...
def data_updater():
    ...
    return updateddf, expirydf, updatedeqdf

def weekly_short_backtest():
    optdf, expirydf, eqdf = data_updater()
    expiry_df = expiry_changes(expirydf.copy())
    opt_df = ticker_change(optdf.copy(),'Y')
    ...
    weeklydf = extract_currentweek_contracts_from_bhavcopy(opt_df.copy())
    weeklydf = merge_spot(weeklydf.copy(),eqdf.copy())
    firstweek_entry = entry_on_first_week(weeklydf.copy())
    entrydf = getentrydf(firstweek_entry.copy())
    atmdf_entry = strike_selector(firstweek_entry.copy())
    atmdf_entry,atmdf_exit = getexitdf(weeklydf.copy(),atmdf_entry.copy(),'Short')
...
def nav_calculation(tradebook):
    ...
    def calculate_nav():
        ...
        base_dir = rf"C:\Vishwanath\PythonCodes\Strategy\MonthlyBuying\{index}\NAVs\Updated_{updated_asof}"
        os.makedirs(base_dir, exist_ok=True)

        output_path = os.path.join(base_dir,f"{index.capitalize()}_ShortStraddle_{timeframe.capitalize()}_NAV.xlsx")
        nav.to_excel(output_path)
    
    calculate_nav()

def deltaCalculator():
    df = pd.read_csv(rf"C:\Vishwanath\PythonCodes\Strategy\MonthlyBuying\{index}\Tradebook\Updated_{updated_asof}\{index}_{type}Straddle_{timeframe}.csv")
    nav = pd.read_excel(rf"C:\Vishwanath\PythonCodes\Strategy\MonthlyBuying\{index}\NAVs\Updated_{updated_asof}\{index}_{type}Straddle_{timeframe}_NAV.xlsx")
    ...
    def calculate_delta(df,nav):
        ...
        os.makedirs(rf"C:\Vishwanath\PythonCodes\Strategy\MonthlyBuying\{index}\Delta\LongShort\Updated_{updated_asof}\\",exist_ok=True)
        ddf.to_excel(rf"C:\Vishwanath\PythonCodes\Strategy\MonthlyBuying\{index}\Delta\LongShort\Updated_{updated_asof}\{index}_{position}StraddleDelta.xlsx")
    
    calculate_delta(df.copy(),nav.copy())

if __name__ == '__main__':
    print(f"Running Backtest for {index.capitalize()} Weekly Short Straddle")
    short_tradebook = weekly_short_backtest()
    print(short_tradebook['PnL_after_commission'].sum())
    os.makedirs(fr"C:\Vishwanath\PythonCodes\Strategy\MonthlyBuying\{index}\Tradebook\Updated_{updated_asof}",exist_ok=True)
    short_tradebook.to_csv(fr"C:\Vishwanath\PythonCodes\Strategy\MonthlyBuying\{index}\Tradebook\Updated_{updated_asof}\{index.capitalize()}_ShortStraddle_Weekly.csv",index=False)
    print("BACKTEST UPDATED!!")
    print("GENERATING NAVs for WEEKLY SHORT STRADDLE STRATEGY")
    nav_calculation(short_tradebook.copy())
    print(f"CALCULATING DELTA FOR {index} WEEKLY SHORT STRADDLE STRATEGY")
    deltaCalculator()
    print("COMPLETED!!")
