# -*- coding: utf-8 -*-
"""
Created on Wed Oct 12 14:41:07 2022

@author: Viren@Incred
Testing SeasoningLong:
...
class SeasoningLong(FactoryBackTester):
    ...
    def basicdatainitialize(self):
        ClosePrice = self.BackTestData.Close.loc[:, self.ticker].dropna()
        ...
        self.UpdateDates = list(self.BackTestData.Close.index)#list(self.BackTestData.Close.index)#.values()
    ...
    def UpdateSpecificStats(self):
        if numpy.isnan(self.BackTestData.Close.loc[self.CurrentTime, self.ticker]):
            self.order.symbol = self.ticker
            self.order.segment = Segment.EQ
            self.order.quantity = 1
            if self.Position.loc[self.LastTime, self.ticker] in [1, -1]:
                ...
                next
        elif self.CurrentTime.day not in range(5, 25):
...
if __name__=='__main__':
    ...
    f.close()
    basePath = dirPath+'BacktestsResults/DailyTradingSignals/SeasoningNiftyRatioLong_'
    ...
    mydata.Close = mydata.Close.div(mydata.indexprice.loc[:, 'NZ1 INDEX'], axis = 0)
    for ticker in mydata.Close.columns:
        try:
            a = SeasoningLong(mydata, ticker)
            a.run()
            print(ticker)
