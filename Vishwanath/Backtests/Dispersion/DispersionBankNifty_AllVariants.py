# -*- coding: utf-8 -*-
"""
Created on Wed Apr  5 14:17:05 2023

@author: Viren@InCred

Nifty/bank Nifty Dispersion Model// If Index Moves more than by some %, then Take the new positions.
1. Completely New Straddle
2. Keep for stocks same but for Index Strikes change it
3. Design some IV Spread based Model
"""
...
class DispersionNiftyIndexMoves(FactoryBackTester):
    def __init__(self,data, strategyName):
    ...
    def basicdatainitialize(self):
        self.CurrentTime = pandas.to_datetime(list(self.BackTestData.TradingDates.keys())[1])#pandas.to_datetime('2014-06-26')
        #self.EndTime = pandas.to_datetime('2022-12-29')
        self.UpdateDates = list(self.BackTestData.TradingDates.keys())#self.ExpiryDates
    ...
    def UpdateSpecificStats(self):
        if self.CurrentTime < self.EndTime:
            StocksCount = 6
            ticker = 'BANKNIFTY'
            #activeTickers = self.BackTestData.TradingDates[self.CurrentTime.date()]
            indexComp = [self.BackTestData.DictBloomNSE[it] for it in self.GetLatestIndexComponents()]
            curMcapWt = self.BackTestData.Index.MarketCap.loc[self.CurrentTime]
            #curMcapWt = curMcap.loc[curMcap.index.isin(indexComp)].dropna()
            # if self.CurrentTime in self.ExpiryDates or self.CurrentTime == datetime.date(2023, 6, 28):
            ...
            # if self.NearExpiryDate == datetime.datetime(2014, 4, 23):
            #     self.NearExpiryDate = datetime.datetime(2014, 4, 24)
            ...
            #self.CurTickersWt = []
            #universe = set.intersection(set(self.BackTestData.TradingDates[self.CurrentTime.date()]), set(self.Close.columns))
            universe = self.BackTestData.TradingDates[self.CurrentTime.date()]
            self.CurTickersWt = pandas.DataFrame([re.match(self.OptionTickerRegEx, iTicker).groupdict() for iTicker in universe], index = universe)
            self.CurTickersWt.strike = self.CurTickersWt.strike.astype('float')
            AvgStrike = self.CurTickersWt.groupby('symbol').mean('strike')
            self.CurTickersWt['AvgStrike'] = [AvgStrike.loc[it, 'strike'] for it in self.CurTickersWt.symbol]
            
            curMcapWt = curMcapWt.loc[curMcapWt.index.isin(self.CurTickersWt.symbol)]#.sort_values().iloc[-StocksCount:]
            curMcapWt = curMcapWt/curMcapWt.sum()
            curMcapWt = -1*curMcapWt
            curMcapWt = pandas.concat([curMcapWt, pandas.DataFrame([1.0], index = [ticker])], axis = 0)
            
            try:
                self.CurTickersWt['Weight'] = [curMcapWt.loc[it, 0] for it in self.CurTickersWt.symbol]
            except:
                pdb.set_trace()
    ...
    def updateCapAllocation(self):
        ...
        self.UpdateOrderBook(strategyID = self.StrategyName, options = 'y')
...
if __name__=='__main__':
    ...
    if os.path.exists('Z:/Pickles/'):
        #pickleFile = f'Z:/Pickles/BNF_Disp_{strategy_names[strategyNum]}_{datetime.datetime.today().date().strftime("%d%b-%Y") +".pkl"}'
        pickleFile = f'Z:/Pickles/BNF_Disp_{strategy_names[strategyNum]}_29Jan-2025.pkl'
        #'Z:/Pickles/BNF_Disp_MoveStraddleifBNFMoves04Oct-2023.pkl'
    else:
        pickleFile = 'G:/Shared drives/BackTests/Pickles/BNF_Disp_MoveStraddleifBNFMoves04Oct-2023.pkl'
    ...
    model = DispersionNiftyIndexMoves(mydata, strategyName = strategy_names[strategyNum])
    model.run()
    ...
    if not os.path.exists(basePath):
        ...
        #os.mkdirs(basePath)       
    backtestName = f'BNFDisp_{strategy_names[strategyNum]}_'
