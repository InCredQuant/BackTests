...
@author: Viren@InCred
Backtesting- IronCOndor on Bank Nifty Strikes
"""
...
class IronCondorBankNifty(FactoryBackTester):
    ...
    def StopLossHandler(self):
        self.StopLoss_Advanced(byGroup = True, StopLossLimit = -0.35)
        self.UpdateOrderBook(strategyID = 'IronCondorBankNifty_4_10pct', options = 'y')
...
if __name__=='__main__':
    ...
    if os.path.exists('Z:/Pickles/'):
        pickleFile = 'Z:/Pickles/BankNiftyIronCondorData29Mar-2023.pkl'
    else:
        pickleFile = 'G:/Shared drives/BackTests/Pickles/BankNiftyIronCondorData29Mar-2023.pkl'
    ...
    model = IronCondorBankNifty(mydata)
    model.run()
            
    
    basePath = 'G:/Shared drives/BackTests/BackTestsResults/IronCondor/BankNifty/'
    if not os.path.exists(basePath):
        path = Path(basePath)
        path.mkdir(parents=True)
        #os.mkdirs(basePath)       
    backtestName = 'BankNiftyIronCondor_4_10pct_35pctSL_'
