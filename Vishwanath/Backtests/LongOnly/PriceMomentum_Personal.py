...
## Finalisd:
    1. 6 Months Price Momentum with 3 Months Postive Absolute price for entry, exit out of 15-30, BSE500 Index Universe
    2. 6 Months Price Low Volatility, with 6 Months absolute price for Entry,  exit out of 15-25, BSE100 Index Universe
"""
...
class PriceMomentum(FactoryBackTester):
    def __init__(self,data, indexName, modelName, rebalType = 'm', stocksCount = [15, 25]):
    ...
    def basicdatainitialize(self):
        self.CurrentTime = min(list(self.BackTestData.IndexCompDict[self.IndexName].keys())[6:]) + datetime.timedelta(1)#
        #self.CurrentTime = pandas.to_datetime('2015-05-02')
        self.CurrentTime = pandas.to_datetime('2024-07-31')
    ...
    def UpdateSpecificStats(self):
        ...
        self.PriceChk = self.Close.pct_change(63).loc[self.CurrentTime, namesinIndex].dropna()
        self.PriceChk = self.PriceChk[self.PriceChk >0]        
        ...
        if self.ModelName == '1YrPriceMomVolAdj':
            ...
            self.PriceChk = self.Close.pct_change(126).loc[self.CurrentTime, namesinIndex].dropna()
            self.PriceChk = self.PriceChk[self.PriceChk >0]
            self.FactorRanks = PriceVol
    ...
    def updateCapAllocation(self):
        if len(self.FactorRanks) >= 0:
            ...
            if self.ModelName == 'MonthlyMACD':
                ...
                self.CapitalAllocation.loc[self.CurrentTime, set.difference(set(self.Close.columns), commontolast)] = 0
                if len(commontolast) == mincnt:
                    ...
                    newEntry = set.intersection(set(newEntry), set(self.PriceChk.index))
                    longStocks = list(set.union(set(commontolast), set(newEntry)))                    
                    if mincnt - len(commontolast) > 0:
                        self.CapitalAllocation.loc[self.CurrentTime, newEntry] = (self.CurrentNAV - self.CapitalAllocation.loc[self.CurrentTime,  commontolast].sum())/(mincnt - len(commontolast))#len(longStocks)
    ...
    def updateCapAllocation_old(self):
        if len(self.FactorRanks) >= 0:
            self.CapitalAllocation.loc[self.CurrentTime] = 0
            self.Position.loc[self.CurrentTime] = 0
...
if __name__=='__main__':
    ...
    #f = open(picklePath+ '_AllCap20240804.pkl', 'rb')#datetime.datetime.today().date().strftime('%Y%m%d') +'.pkl', 'rb')
    ...
    Category = rebalType.capitalize()+'Rebal_'
    #basePath = os.path.join('C:/Users/virendra.kumar_incre/Desktop/Personal/Trading/BackTests/', Category +'LiveModel'+ datetime.datetime.now().date().strftime('_%d%b%Y/'))
    basePath = os.path.join('G:/Shared drives/QuantFunds/EquityPlus/Backtests/Momentum/Viren/', Category + datetime.datetime.now().date().strftime('_%d%b%Y/'))
    if not os.path.exists(basePath):
        os.makedirs(basePath)
    
    indices = {1 : 'NIFTY', 2: 'BSE100', 3: 'BSE200', 4: 'BSE500', 5: 'SPBSAIP', 6: 'BSEALL'}
    numStocks = {'NIFTY': [15, 20], 'BSE100': [15, 25], 'BSE200': [15, 25], 'BSE500': [15, 30], 'SPBSAIP': [15, 25], 'BSEALL' : [30, 50]}
    portFileName = basePath + 'ModelsData.xlsx'
    #ModelNum = {'3MPriceMom' : [6], '1YrPriceMomVolAdj': [6], '6MPriceMom' : [6], '1YrPriceMom' : [6], 'DrawDownAvgLow': [6], 'MonthlyMACD': [6], 'Near52Wkhigh_NrTop15Pct': [6]}
    ModelNum = {'6MPriceMom' : [4, 6]}
    #ModelNum = {'1YrPriceMomVolAdj': [2, 3, 4], '6MPriceMom' : [2, 3, 4], '1YrPriceMom' : [2, 3, 4], '6MPriceLowVol': [2, 3, 4], '3MPriceMom' : [2, 3, 4], '1YrPriceLowVol' : [2, 3, 4]}
