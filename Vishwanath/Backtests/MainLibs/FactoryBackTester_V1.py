...
We have moved to Different Entry and Exit Price// not just Clsoe Price
...
class FactoryBackTester():
    ...
    def __init__(self, data):
        ...
        self.EntryPrice = copy.deepcopy(data.Close)# it will be used for Entry Price, if we have to Enter at some particular level other than Close Price, We can use Open Price, Low Price or High Price or some other level
        self.ExitPrice = copy.deepcopy(data.Close)# it will be used for Exit Price, if we have to Exit at some particular level other than Close Price, We can use Open Price, Low Price or High Price or some other level
        ...
        self.TransactionCostRate=0.000#25#2.5 bps
    ...
    def GetLatestIndexComponents(self):
        try:
            return self.BackTestData.Index.indexcomponents[max([i for i in self.BackTestData.Index.indexcomponents.keys() if i < self.CurrentTime])]
        except:
...
#            self.orderhandler()
    ...
    def updatePnLandNAVhandler(self):
        ...
        if len(self.TempCapitalAllocation)!=0:
            ...
            #LongCurrentPriceRatio = numpy.divide(self.Close.loc[self.CurrentTime],self.Close.loc[self.LastTime])# based on Clsoe price
            LongCurrentPriceRatio = numpy.divide(self.ExitPrice.loc[self.CurrentTime],self.EntryPrice.loc[self.LastTime])# Based on Entry and Exit price, Entry & Exit Prices might change depending on the Stop Loss or target price
            #CurrentPriceRatio = numpy.divide(self.Close.loc[self.LastTime], self.Close.loc[self.CurrentTime])
            CurrentPriceRatio = numpy.divide(self.EntryPrice.loc[self.LastTime], self.ExitPrice.loc[self.CurrentTime])# Based on Entry and Exit Prices
...
#        self.drawdownstoplosshandler()
    ...
    def updatePnLandNAVhandlerOptions(self):
        ...
        if len(self.TempCapitalAllocation)!=0:
            ...
            #First carry forward the last Quantity position, if not overwriten otherwise
            #pdb.set_trace()
            ...
            #CalQty = pandas.concat([CalQty, tempDF], axis =1)
            #if self.CurrentTime.date() == datetime.date(2014, 8, 1):
            #    pdb.set_trace()
            ...
            self.TradeLog.loc[self.LastTime,:]= numpy.subtract(self.YestDayEnd,self.TempCapitalAllocation)
            # today before trade is change is allocated capital due to price change. present value of allocated capital due to effect of market activity
            LongCurrentPriceRatio = numpy.divide(self.ExitPrice.loc[self.CurrentTime],self.EntryPrice.loc[self.LastTime])
            CurrentPriceRatio = numpy.divide(self.EntryPrice.loc[self.LastTime], self.ExitPrice.loc[self.CurrentTime])
            LongCurrentPriceRatio = LongCurrentPriceRatio.where(~numpy.isnan(LongCurrentPriceRatio), other = 1)#, inplace =True)#CurrentPriceRatio.where(~numpy.isnan(CurrentPriceRatio), other = 1)#
            CurrentPriceRatio = CurrentPriceRatio.where(~numpy.isnan(CurrentPriceRatio), other = 1)#, inplace = True)
            ...
            self.CapitalAllocation.loc[self.CurrentTime,:]=(self.TodayBeforeTrade)
            # mtm due to last days capital allocation
            PriceChg = numpy.subtract(self.ExitPrice.loc[self.CurrentTime, :], self.EntryPrice.loc[self.LastTime, :])
            PriceChg = PriceChg.where(~numpy.isnan(PriceChg), other = 0)            
            self.MTM.loc[self.CurrentTime, :] = numpy.multiply(PriceChg, CalQty)
            
            #self.MTM.loc[self.CurrentTime,:]=numpy.subtract(numpy.abs(self.TodayBeforeTrade),numpy.abs(self.YestDayEnd))
            self.TransactionCost.loc[self.CurrentTime,'TransactionCost']=numpy.sum((numpy.abs(self.TradeLog.loc[self.LastTime])))*(self.TransactionCostRate)
            tempChg = 1 + numpy.divide(numpy.sum(self.MTM.loc[self.CurrentTime]), GrossExposure.abs().sum())
...
#                    self.AllOrders.remove(order)
    ...
    def savebacktestresult_minimal(self, filepath, fullData=True):
        t1 = time.time()
    ...
    def OrderPosition(self, iTicker, entry_date, position, strategyID = '', exit_date='', options = None):
        ...
        self.order.entry_price = self.EntryPrice.loc[entry_date, iTicker]
        if exit_date != '':
        ...
        self.order.exit_price = self.ExitPrice.loc[exit_date, iTicker]
        ...
        try:
            ...
            if (position == 1 and AwayFromLow < StopLossLimit) or (position == -1 and AwayFromHigh < StopLossLimit):
                ...
                if position == 1:
                    ...
                    self.ExitPrice.loc[self.CurrentTime, ticker] = (1+ StopLossLimit*position)*max(self.Close.loc[TradeTakenDate, ticker], self.BackTestData.High.loc[TradeTakenDate+datetime.timedelta(1):self.CurrentTime, ticker].max())
                elif position == -1:
                    self.ExitPrice.loc[self.CurrentTime, ticker] = (1+ StopLossLimit*position)*min(self.Close.loc[TradeTakenDate, ticker], self.BackTestData.Low.loc[TradeTakenDate+datetime.timedelta(1):self.CurrentTime, ticker].min())
        except:
    ...
    def StopLoss(self, ticker, StopLossLimit):
         ...
         try:
             ...
             if rets*self.PositionWOSL.loc[self.CurrentTime, ticker] < StopLossLimit:
                 self.PositionExitDF.loc[self.CurrentTime, ticker] = -1
                 self.ExitPrice.loc[self.CurrentTime, ticker] = (1+ StopLossLimit*self.PositionWOSL.loc[self.CurrentTime, ticker])*self.Close.loc[TradeTakenDate, ticker]
                 # it replaces the Close Price Data to the Stop Loss Price
    ...
    def Target(self, ticker, TargetLimit):
         ...
         try:
             ...
             if rets*self.PositionWOSL.loc[self.CurrentTime, ticker] >= TargetLimit:
                 self.PositionExitDF.loc[self.CurrentTime, ticker] = -1
                 self.ExitPrice.loc[self.CurrentTime, ticker] = (1+ TargetLimit*self.PositionWOSL.loc[self.CurrentTime, ticker])*self.Close.loc[TradeTakenDate, ticker]
    ...
    def ExpiryTickers(self, tickers):
        return [it+self.NearExpiry+'XX0' for it in tickers] # it append the Tickers with Latest Expiry
    
    def InvExpiryTickers(self, tickers):
        return [it.replace(self.NearExpiry+'XX0', '') for it in tickers]
    ...
    def MD1_RSIwith50SMATrend(self, tickersList):
        ...
        Description:
        ------------
        This code is a method in a class that implements a trading strategy based on the relative strength index (RSI) and 50-day simple moving average (SMA). The method takes a single argument tickersList, which is a list of tickers for which the strategy is to be applied.
        The method first initializes a dictionary called StartingPosition, which will be used to store the starting positions for each ticker. If the StartingPosition attribute does not yet exist, the method creates it and sets the starting position for each ticker in tickersList to -1. If the StartingPosition attribute already exists, the method adds any tickers in tickersList that are not already in the dictionary and sets their starting positions to -1.
        The method then retrieves the RSI and 50-day SMA data for the tickersList from the BackTestData dataframe, and the close prices for the current time step from the Close dataframe. It updates the SMA50Position dataframe to reflect whether the current close price for each ticker is above or below its 50-day SMA.
        The method then defines a set of tickers for which the last RSI signal was negative or the starting position is negative, and a set of tickers for which the last RSI was less than 10 and the current RSI is greater than 10. It updates the RSI2Position dataframe to reflect a positive RSI signal for these tickers, and also updates the StartingPosition dictionary to reflect a neutral starting position for these tickers.
        The method then defines a set of tickers for which the last RSI signal was positive, and a set of tickers for which the last RSI was greater than 90 and the current RSI is less than 90. It updates the RSI2Position dataframe to reflect a negative RSI signal for these tickers.
        Finally, the method updates the RSI2Position dataframe for tickers that do not fall into any of the above categories, setting their positions to the same as in the previous time step or to negative if their starting position is negative. The method also updates the Strategy dataframe to reflect the strategy being used for each ticker at the current time step.
        '''
        if not hasattr(self, 'StartingPosition'):
            ...
            for ticker in tickersList:
                if ticker not in self.StartingPosition.keys() or numpy.isnan(self.BackTestData.Index.SMA50.loc[self.CurrentTime, ticker]):
                    self.StartingPosition[ticker] = -1
        # Defining Strategy
        if hasattr(self, 'Strategy'):
            #for ticker in tickersList:
            self.Strategy.loc[self.CurrentTime, self.ExpiryTickers(tickersList)] = 'RSIwith50SMATrend'
                
        RSI = self.BackTestData.Index.RSI2.loc[:, tickersList]
        SMA = self.BackTestData.Index.SMA50.loc[self.CurrentTime, tickersList]
        Price = self.BackTestData.Index.Close.loc[self.CurrentTime, tickersList]
        ...
        SignalTickerswithExp = self.ExpiryTickers(SignalTickers)#set([it + self.NearExpiry +'XX0' for it in SignalTickers])
        tickersListwithExp = self.ExpiryTickers(tickersList)#set([it + self.NearExpiry +'XX0' for it in tickersList])
        
        if len(SignalTickers)>0:
            tempdf = self.SMA50Position.loc[self.CurrentTime, SignalTickers]
            tempdf.index = self.ExpiryTickers(tempdf.index)
            self.PositionWOSL.loc[self.CurrentTime, SignalTickerswithExp] = tempdf#.values#[0]
        self.PositionWOSL.loc[self.CurrentTime, set(tickersListwithExp).difference(set(SignalTickerswithExp))] = 0
        
        for iTicker in SignalTickerswithExp:
    ...
    def MD2_RSI50SMA(self, tickersList):
        ...
        if hasattr(self, 'Strategy'):
            #for ticker in tickersList:
            self.Strategy.loc[self.CurrentTime, self.ExpiryTickers(tickersList)] = 'RSI50SMA'
        ...
        if len(SignalTickers)>0:
            tempdf = self.RSI2Position.loc[self.CurrentTime, SignalTickers]
            tempdf.index = self.ExpiryTickers(tempdf.index)
            self.PositionWOSL.loc[self.CurrentTime, self.ExpiryTickers(SignalTickers)] = tempdf#.values[0]
        ...
        RemainTickersMD2 = set(tickersList).difference(SignalTickers)
        #RemainTickersMD2withExp = set([it + self.NearExpiry +'XX0' for it in RemainTickersMD2])
        #Last Position is not matching to Current SMA 50 Position
        
        #tempPosSMA50 = self.SMA50Position.loc[self.CurrentTime,:]
        #tempPosSMA50.index = self.ExpiryTickers(tempPosSMA50.index)#[it+ self.NearExpiry +'XX0' for it in tempPosSMA50.index]
        #tempPosRSI2 = self.RSI2Position.loc[self.CurrentTime,:]
        #tempPosRSI2.index = [it+ self.NearExpiry +'XX0' for it in tempPosRSI2.index]
        ...
        tempPos = self.SMA50Position.loc[self.CurrentTime,RemainTickersMD2]
        tempPos.index = self.ExpiryTickers(tempPos.index)
        posChk = self.PositionWOSL.loc[self.LastTime, self.ExpiryTickers(RemainTickersMD2)] != tempPos
        posChk.index = self.InvExpiryTickers(posChk.index)#[it.replace(self.NearExpiry+'XX0', '') for it in posChk.index]#
        cond1Tickers = Price.loc[RemainTickersMD2][posChk].dropna().index
        ...
        tempPosdf = self.PositionWOSL.loc[self.LastTime, self.ExpiryTickers(RemainTickersMD2)]
        tempPosdf.index = self.InvExpiryTickers(tempPosdf.index)        
        cond3Tickers = Price.loc[RemainTickersMD2][self.RSI2Position.loc[self.CurrentTime, RemainTickersMD2] != tempPosdf].dropna().index
        #conditionTickers = {}
        conditionTickers = set(cond1Tickers).union(set.intersection(set(cond2Tickers), set(cond3Tickers)))
        if len(conditionTickers)>0:
            self.PositionWOSL.loc[self.CurrentTime, self.ExpiryTickers(conditionTickers)] = 0
        self.PositionWOSL.loc[self.CurrentTime, self.ExpiryTickers(RemainTickersMD2.difference(conditionTickers))] = self.PositionWOSL.loc[self.LastTime, self.ExpiryTickers(RemainTickersMD2.difference(conditionTickers))]
    ...
    def MD3_Series3(self, tickersList):
        ...
        if hasattr(self, 'Strategy'):
            #for ticker in tickersList:
            self.Strategy.loc[self.CurrentTime, self.ExpiryTickers(tickersList)] = 'Series3'
        SMA = self.BackTestData.Index.SMA100.loc[self.CurrentTime, tickersList]
        Price = self.BackTestData.Index.Close.loc[self.CurrentTime, tickersList]
        ...
        tempPosdf = self.PositionWOSL.loc[self.LastTime, self.ExpiryTickers(tickersList)]
        tempPosdf.index = self.InvExpiryTickers(tempPosdf.index)
        SignalTickers = set.intersection(set(Price[tempPosdf == 0].dropna().index), set(Price[self.DirectionPosition.loc[self.CurrentTime, tickersList] ==1].dropna().index))        
        tempSMAPos = self.SMA100Position.loc[self.CurrentTime, SignalTickers]
        tempSMAPos.index = self.ExpiryTickers(tempSMAPos.index)
        self.PositionWOSL.loc[self.CurrentTime, self.ExpiryTickers(SignalTickers)] = tempSMAPos
        RemainTickers = tickersList.difference(SignalTickers)
        if len(RemainTickers) > 0:
            self.PositionWOSL.loc[self.CurrentTime, self.ExpiryTickers(RemainTickers)] = self.PositionWOSL.loc[self.LastTime, self.ExpiryTickers(RemainTickers)]
            for iTicker in list(self.ExpiryTickers(RemainTickers)):
                #self.ticker = iTicker
                ...
                break
                if tradingDays > 15 or self.SMA100Position.loc[self.CurrentTime, self.InvExpiryTickers([iTicker])[0]] != self.PositionWOSL.loc[self.LastTime, iTicker]:
    ...
    def MD4_20WMA_MACD(self, tickersList):
        ...
        if hasattr(self, 'Strategy'):
            #for ticker in tickersList:
            self.Strategy.loc[self.CurrentTime, self.ExpiryTickers(tickersList)] = '20WMA_MACD'
        Price = self.BackTestData.Index.Close.loc[self.CurrentTime, tickersList]        
        ...
        self.PositionWOSL.loc[self.CurrentTime, self.ExpiryTickers(PosSignal)] = 1
        self.PositionWOSL.loc[self.CurrentTime, self.ExpiryTickers(NegSignal)] = -1
        self.PositionWOSL.loc[self.CurrentTime, self.ExpiryTickers(tickersList.difference(PosSignal, NegSignal))] = 0
    ...
    def MD5_BodyOutSideBand(self, tickersList):
        ...
        if hasattr(self, 'Strategy'):
            #for ticker in tickersList:
            self.Strategy.loc[self.CurrentTime, self.ExpiryTickers(tickersList)] = 'BodyOutSideBand'
        Price = self.BackTestData.Index.Close.loc[self.CurrentTime, tickersList]
        High = self.BackTestData.Index.High.loc[self.CurrentTime, tickersList]
        Low = self.BackTestData.Index.Low.loc[self.CurrentTime, tickersList]
        EMA = self.BackTestData.Index.EMA21.loc[self.LastTime, tickersList]
        ...
        if len(LastNeutralCurPos)>0:
            tradeDF = self.TradePosition.loc[self.CurrentTime, LastNeutralCurPos]
            tradeDF.index = self.ExpiryTickers(tradeDF.index)
            self.PositionWOSL.loc[self.CurrentTime, self.ExpiryTickers(LastNeutralCurPos)] = tradeDF #.values
        
        tempPos = self.PositionWOSL.loc[self.LastTime, self.ExpiryTickers(tickersList)]
        tempPos.index = self.InvExpiryTickers(tempPos.index)
        LastPos = Price[ tempPos== 1].dropna().index
        ...
        LastNeg = Price[tempPos == -1].dropna().index
        exitShort = Price.loc[LastNeg][Low.loc[LastNeg] > 0.99*EMA.loc[LastNeg]].dropna().index
        self.PositionWOSL.loc[self.CurrentTime, self.ExpiryTickers(exitLong.union(exitShort))] = 0# Exited both Long and Short which goies outside band Range
        self.PositionWOSL.loc[self.CurrentTime, self.ExpiryTickers(LastPos.difference(exitLong).union(LastNeg.difference(exitShort)))] = self.PositionWOSL.loc[self.LastTime, self.ExpiryTickers(LastPos.difference(exitLong).union(LastNeg.difference(exitShort)))]# Carry same Position if Band is not Broken
        ...
        for iTicker in self.ExpiryTickers(tickersList):
            if self.PositionWOSL.loc[self.CurrentTime, iTicker] in [-1, 1]:
                  self.StopLoss(iTicker, -0.03)# -3% StopLoss
                  self.Target(iTicker, 0.10)# +10% Target
    ...
    def MD6_ROCMA(self, tickersList):
        ...
        if hasattr(self, 'Strategy'):
            #for ticker in tickersList:
            self.Strategy.loc[self.CurrentTime, self.ExpiryTickers(tickersList)] = 'ROCMA'
        Price = self.BackTestData.Index.Close.loc[self.CurrentTime, tickersList]
        PosSignal = Price[self.BackTestData.Index.ROC27D.loc[self.CurrentTime, tickersList] > self.BackTestData.Index.ROCMA18D.loc[self.CurrentTime, tickersList]].dropna().index
        NegSignal = Price[self.BackTestData.Index.ROC27D.loc[self.CurrentTime, tickersList] <= self.BackTestData.Index.ROCMA18D.loc[self.CurrentTime, tickersList]].dropna().index
        self.PositionWOSL.loc[self.CurrentTime, self.ExpiryTickers(PosSignal)] = 1
        self.PositionWOSL.loc[self.CurrentTime, self.ExpiryTickers(NegSignal)] = -1
    ...
    def MD7_RegressionCrossOver(self, tickersList):
        ...
        if hasattr(self, 'Strategy'):
            #for ticker in tickersList:
            self.Strategy.loc[self.CurrentTime, self.ExpiryTickers(tickersList)] = 'RegressionCrossOver'
        ...
        Price = self.BackTestData.Index.Close.loc[self.CurrentTime, tickersList]
        SMA = self.BackTestData.Index.SMA10.loc[self.CurrentTime, tickersList]
        LS = self.BackTestData.Index.LS.loc[self.CurrentTime, tickersList]
        LR = self.BackTestData.Index.LR.loc[self.CurrentTime, tickersList]
        ...
        PrevPos = Price[tempPos == 1].dropna().index
        PrevNeg = Price[tempPos == -1].dropna().index
        exitPos = PrevPos.intersection(belowSMA.union(belowLR))
        exitNeg = PrevNeg.intersection(aboveSMA.union(aboveLR))
        self.PositionWOSL.loc[self.CurrentTime, self.ExpiryTickers(exitPos.union(exitNeg))] = 0
        self.PositionWOSL.loc[self.CurrentTime, self.ExpiryTickers(PrevPos.difference(exitPos))] = self.PositionWOSL.loc[self.LastTime, self.ExpiryTickers(PrevPos.difference(exitPos))]
        self.PositionWOSL.loc[self.CurrentTime, self.ExpiryTickers(PrevNeg.difference(exitNeg))] = self.PositionWOSL.loc[self.LastTime, self.ExpiryTickers(PrevNeg.difference(exitNeg))]
        
        self.PositionWOSL.loc[self.CurrentTime, self.ExpiryTickers(Positive)] = 1
        self.PositionWOSL.loc[self.CurrentTime, self.ExpiryTickers(Negative)] = -1
    ...
    def MD8_Vortex(self, tickersList):
        ...
        if hasattr(self, 'Strategy'):
            #for ticker in tickersList:
            self.Strategy.loc[self.CurrentTime, self.ExpiryTickers(tickersList)] = 'Vortex'
        Price = self.BackTestData.Index.Close.loc[self.CurrentTime, tickersList]
        Ratio = self.PviNviRatio.loc[self.CurrentTime, tickersList]
        self.PositionWOSL.loc[self.CurrentTime, self.ExpiryTickers(Price[Ratio>0].dropna().index)] = 1
        self.PositionWOSL.loc[self.CurrentTime, self.ExpiryTickers(Price[Ratio<0].dropna().index)] = -1
    ...
    def MD9_Oscillator(self, tickersList):
        ...
        if hasattr(self, 'Strategy'):
            #for ticker in tickersList:
            self.Strategy.loc[self.CurrentTime, self.ExpiryTickers(tickersList)] = 'Oscillator'
        Price = self.BackTestData.Index.Close.loc[self.CurrentTime, tickersList]
        ...
        ExitLong = set.intersection(set(Price[tempPos == 1].dropna().index), set(Price[self.ATRRatio.loc[self.CurrentTime, tickersList] < 0].dropna().index))
        ExitShort = set.intersection(set(Price[tempPos == -1].dropna().index), set(Price[self.ATRRatio.loc[self.CurrentTime, tickersList] > 0].dropna().index))
        self.PositionWOSL.loc[self.CurrentTime, self.ExpiryTickers(ShortSignal)] = -1
        self.PositionWOSL.loc[self.CurrentTime, self.ExpiryTickers(LongSignal)] = 1
        self.PositionWOSL.loc[self.CurrentTime, self.ExpiryTickers(ExitLong.union(ExitShort))] = 0        
        self.PositionWOSL.loc[self.CurrentTime, self.ExpiryTickers(tickersList.difference(LongSignal, ShortSignal, ExitLong, ExitShort))] = self.PositionWOSL.loc[self.LastTime, self.ExpiryTickers(tickersList.difference(LongSignal, ShortSignal, ExitLong, ExitShort))]
    ...
    def MD10_RSI50(self, tickersList):
        ...
        if hasattr(self, 'Strategy'):
            #for ticker in tickersList:
            self.Strategy.loc[self.CurrentTime, self.ExpiryTickers(tickersList)] = 'RSI50'
        Price = self.BackTestData.Index.Close.loc[self.CurrentTime, tickersList]
        RSI = self.BackTestData.Index.RSI14.loc[self.CurrentTime, tickersList]
        self.PositionWOSL.loc[self.CurrentTime, self.ExpiryTickers(Price[RSI>50].dropna().index)] = 1
        self.PositionWOSL.loc[self.CurrentTime, self.ExpiryTickers(Price[RSI<50].dropna().index)] = -1
    ...
    def MD11_AssymetricDaily(self, tickersList):
        ...
        if hasattr(self, 'Strategy'):
            #for ticker in tickersList:
            self.Strategy.loc[self.CurrentTime, self.ExpiryTickers(tickersList)] = 'AssymetricDaily'
        Price = self.BackTestData.Index.Close.loc[self.CurrentTime, tickersList]
        ...
        self.PositionWOSL.loc[self.CurrentTime, self.ExpiryTickers(Long)] = 1
        self.PositionWOSL.loc[self.CurrentTime, self.ExpiryTickers(Short)] = -1
    ...
    def MD11_AssymetricWeekly(self, tickersList):
        ...
        if hasattr(self, 'Strategy'):
            #for ticker in tickersList:
            self.Strategy.loc[self.CurrentTime, self.ExpiryTickers(tickersList)] = 'AssymetricWeekly'
        Price = self.BackTestData.Index.Close.loc[self.CurrentTime, tickersList]
    ...
    def MD12_Seasoning(self, tickersList, sl = -0.05, target = 0.15):
        ...
        if hasattr(self, 'Strategy'):
            #for ticker in tickersList:
            self.Strategy.loc[self.CurrentTime, self.ExpiryTickers(tickersList)] = 'Seasoning'
        Price = self.BackTestData.Index.Close.loc[self.CurrentTime, tickersList]
        Long = set(Price.dropna().index)
        if self.CurrentTime.date() in self.ActiveDaysList:
            self.PositionWOSL.loc[self.CurrentTime, self.ExpiryTickers(Long)] = 1
        else:
            self.PositionWOSL.loc[self.CurrentTime, self.ExpiryTickers(Long)] = 0
