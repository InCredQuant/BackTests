# -*- coding: utf-8 -*-
"""
Created on Thu Mar 23 13:56:36 2023

@author: Viren@InCred
Backtesting- Open Interest Based Strategy 
"""
...
class OpenInterestNifty(FactoryBackTester):
    ...
    def basicdatainitialize(self):
        self.CurrentTime = pandas.to_datetime('2014-06-26')
        self.EndTime = pandas.to_datetime('2022-12-29')
        self.UpdateDates = self.Close.index#self.ExpiryDates
    ...
    def UpdateSpecificStats(self):
        if self.CurrentTime < self.EndTime:            
            ...
            curOI = self.BackTestData.OpenInterest.loc[self.CurrentTime].dropna()
            
            tickersList = []
            tempList = [] # OI data according to scrips and tickers
            
            '''
            for iTicker in temp:
                ok = [tickersList.append([iTicker, s, iTicker[:len(iTicker)-9], s.replace(iTicker, '')]) for s in self.Close.columns if iTicker in s]
                ok = [tempList.append([s, iTicker, curOI.loc[s]]) for s in curOI.index  if iTicker in s]
            '''   
            hedge = True
            if hedge:                
                indTickers = [ticker+self.NearExpiryDate.strftime('%d%b%y').upper()+x for x in ['CE', 'PE']]
                for iTicker in indTickers:
                    ok = [tickersList.append([iTicker, s, iTicker[:len(iTicker)-9], s.replace(iTicker, '')]) for s in self.Close.columns if iTicker in s]
                    ok = [tempList.append([s, iTicker, curOI.loc[s]]) for s in curOI.index  if iTicker in s]
            ...
            tempDF = pandas.DataFrame(tempList, columns = ['Ticker', 'Scrip', 'OI'])
            tempDF.index = tempDF.Ticker
            del tempDF['Ticker']
            
            tickers = [tempDF['OI'][tempDF.Scrip == it].idxmax() for it in list(set(tempDF.Scrip))]
            self.CurTickersWt = self.CurTickersWt.loc[self.CurTickersWt.index.isin(tickers)]
            '''
            curMcapWt = curMcapWt.loc[curMcapWt.index.isin(self.CurTickersWt.NSE)]
            
            curMcapWt = pandas.DataFrame(-curMcapWt/curMcapWt.sum())
            curMcapWt.columns = ['Wt']
            '''
            if hedge:
                temp = pandas.DataFrame([ticker, -1.0], index = ['tick', 'Wt']).transpose()
                temp.index = temp.tick
                del temp['tick']            
                curMcapWt = temp#pandas.concat([curMcapWt, temp], axis = 0)
            
            self.CurTickersWt['Weight'] = [curMcapWt.loc[it, 'Wt'] for it in list(self.CurTickersWt.NSE)]
            
            EqualWt = False
            if EqualWt:
                self.CurTickersWt['Weight'] = [(-1.0/StocksCount if curMcapWt.loc[it, 'Wt'] < 0 else 1.0) for it in list(self.CurTickersWt.NSE) ]
                
            
            self.CurTickersWt['Quantity'] = numpy.divide(self.CurTickersWt.Weight*self.TradedValue, self.CurTickersWt.Strike).round()
    ...
    def updateCapAllocation(self):
        ...
        self.UpdateOrderBook(strategyID = 'OINiftyStrikesShort', options = 'y')
...
if __name__=='__main__':
    ...
    if os.path.exists('Z:/Pickles/'):
        pickleFile = 'Z:/Pickles/NiftyStocksOIData24Mar-2023.pkl'
    else:
        pickleFile = 'G:/Shared drives/BackTests/Pickles/NiftyStocksOIData24Mar-2023.pkl'
    ...
    model = OpenInterestNifty(mydata)
    model.run()
            
    
    basePath = 'G:/Shared drives/BackTests/BackTestsResults/OI/Nifty/'
    if not os.path.exists(basePath):
        path = Path(basePath)
        path.mkdir(parents=True)
        #os.mkdirs(basePath)       
    backtestName = 'OIBased_NiftyStrikesShort_'
