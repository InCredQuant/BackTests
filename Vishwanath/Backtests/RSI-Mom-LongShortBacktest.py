...
class ModelPort(FactoryBackTester):
    ...
    def basicdatainitialize(self):        
        self.CurrentTime = pandas.to_datetime('2012-10-04')# '2013-01-07'
        ...
        self.TransactionCostRate = 0.0002 # 2 bps charges each side
    ...
    def declarecurrentvariables(self):
        ...
        if self.rsiDays == 5:
            self.RSI = self.BackTestData.RSI5.resample('w-THU', convention = 'end').last()
        elif self.rsiDays == 14:
            self.RSI = self.BackTestData.RSI14.resample('w-THU', convention = 'end').last()
    ...
    def updateCapAllocation(self):
        if len(self.factor) > 0:
            ...
            shortsecs = self.factor[:cnt].index
            
            for ticker in longsecs:
                self.CapitalAllocation.loc[self.CurrentTime,ticker]= self.CurrentNAV/(len(longsecs)+ len(shortsecs))
                self.Position.loc[self.CurrentTime, ticker] = 1
            for ticker in shortsecs:
                self.CapitalAllocation.loc[self.CurrentTime,ticker]= -self.CurrentNAV/(len(longsecs)+ len(shortsecs))
                self.Position.loc[self.CurrentTime, ticker] = -1
...
if __name__=='__main__':
    import pickle
    dataFile = 'Z:/Pickles/BSE100_RSI_DMAFutData03Oct-2022.pkl'
    f = open(dataFile, 'rb')
    ...
    rsiDays = {5 : '5D', 14 : '14D'}
    basePath = 'Z:\BacktestsResults\RSI+MOM/'
    ...
    for rPeriod in rebal.keys():
        for mDay in MomDays.keys():
            for rsiDay in rsiDays.keys():
                ...
                backtestName = 'BSE100-'+HighLow[hl2]+'RSI('+rsiDays[rsiDay]+')-'+HighLow[hl]+'Mom('+MomDays[mDay]+')-OtherShort'+rebal[rPeriod]+'withFee'
    ...
    finalDF = pandas.DataFrame(cagrData, columns =['Des', 'HighLow-MoM', 'MomDays', 'HighLow-RSI', 'RSIDays', 'RebalanceFreq', 'StrategyCAGR', 'IndexCAGR', 'Churn'])#.to_clipboard()