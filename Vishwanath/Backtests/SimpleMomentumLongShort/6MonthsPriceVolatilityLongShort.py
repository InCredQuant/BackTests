...
class SimpleMomenutmLongShort(FactoryBackTester):
    ...
    def basicdatainitialize(self):
        self.CurrentTime = pandas.to_datetime('2022-12-27')#('2022-12-27')#('2012-12-27')
    ...
    def GetMomRanks(self, ClosePrice, stocksList):       
        ...
        OneYrStd = ClosePrice.pct_change(fill_method = None).loc[:self.CurrentTime, stocksList].iloc[-252:].std()*numpy.sqrt(252)        
        ...
        SixMonthsStd = ClosePrice.pct_change(fill_method = None).loc[:self.CurrentTime, stocksList].iloc[-126:].std()*numpy.sqrt(252)        
        ...
        #pdb.set_trace()
        self.FactorRanks = SixMonthsStd#pandas.concat([OneYrStd, SixMonthsStd, ThreeMonthsStd], axis =1).mean(axis = 1)
    ...
    def updateCapAllocation(self):
        ...
        self.UpdateOrderBook(strategyID = 'STFMVOL6M')
        print(self.CurrentTime.date(), len(longStocks) + len(shortStocks), 'STFMVOL6M')
...
if __name__=='__main__':
    ...
    #backtestName = 'BSE100Futs_6MVol_LongShort'
