...
class SyntheticCalendarSpread(FactoryBackTester):
    ...
    def UpdateSpecificStats(self):
        if self.CurrentTime <= self.EndTime and self.CurrentTime.time() >= self.PosStartTime and self.CurrentTime.time() <= self.PosEndTime:
            ...
            S1 = futATM + self.Close.loc[self.CurrentTime, self.IndexName+self.ExpiryW1+str(futATM)+'CE'] - self.Close.loc[self.CurrentTime, self.IndexName+self.ExpiryW1+str(futATM)+'PE']
            S2 = futATM + self.Close.loc[self.CurrentTime, self.IndexName+self.ExpiryW2+str(futATM)+'CE'] - self.Close.loc[self.CurrentTime, self.IndexName+self.ExpiryW2+str(futATM)+'PE']            
            if not (numpy.isnan(S1) or numpy.isnan(S2)):
                if self.CurrentSyntheticPosition == 0:
                    ...
                    nearStrikeCall = self.IndexName+self.ExpiryW1+str(futATM)+'CE'
                    nearStrikePut = self.IndexName+self.ExpiryW1+str(futATM)+'PE'
                    nextWkStrikeCall = self.IndexName+self.ExpiryW2+str(futATM)+'CE'
                    nextWkStrikePut = self.IndexName+self.ExpiryW2+str(futATM)+'PE'
                    ...
                    if D1 > (0.09/100) and pct_Diff <= (-0.18/100):#0.04
                        ...
                        tempDF.loc[nearStrikeCall, 'Quantity'] = -150
                        tempDF.loc[nearStrikePut, 'Quantity'] = 150
                        tempDF.loc[nextWkStrikeCall, 'Quantity'] = 150
                        tempDF.loc[nextWkStrikePut, 'Quantity'] = -150
                        #pdb.set_trace()
                        
                    elif D1 < (-0.05/100) and pct_Diff >= (0.05/100):
                    ...
                    S1 = self.ATMStrike + self.Close.loc[self.CurrentTime, self.IndexName+self.ExpiryW1+str(self.ATMStrike)+'CE'] - self.Close.loc[self.CurrentTime, self.IndexName+self.ExpiryW1+str(self.ATMStrike)+'PE']
                    S2 = self.ATMStrike + self.Close.loc[self.CurrentTime, self.IndexName+self.ExpiryW2+str(self.ATMStrike)+'CE'] - self.Close.loc[self.CurrentTime, self.IndexName+self.ExpiryW2+str(self.ATMStrike)+'PE']            
                    if not (numpy.isnan(S1) or numpy.isnan(S2)):
                        D1 = S1/spotValue -1
                        D2 = S2/spotValue -1
                        pct_Diff = D1-D2                        
                        if self.CurrentSyntheticPosition == -1:                            
                            if D1 < (0.03/100):#0.03
                                #current week premium comes below 3bps
                                self.CurrentSyntheticPosition = 0
                                self.MakePositionNil()
                        elif self.CurrentSyntheticPosition == 1:
                            if D1 > (-0.01/100):
                                # Current Week Discount becomes less than 1 bps
                                self.CurrentSyntheticPosition = 0
                                self.MakePositionNil()
                    else:
                        self.CurTickersWt = pandas.DataFrame()
            ...
            self.CurTickersWt = pandas.DataFrame()
        if self.CurrentTime.date() == datetime.datetime.strptime(self.ExpiryW1, '%d%b%y').date() and self.CurrentTime.time() >= datetime.time(15, 10, 59):
            self.MakePositionNil()
...
if __name__=='__main__':    
    ...
    INDEXNAME = 'BANKNIFTY'
    slippage = '1pct'
    strategy = f'SyntheticCalendarSpread_{INDEXNAME.title()}'
    ...
    mydata.ExpiryDates = [iDate for iDate in mydata.ExpiryDates if iDate >= datetime.date(2021, 12, 31)]
    ...
    for iD, iExpDate  in enumerate(mydata.ExpiryDates[:-2]):        
        ...
        StrikeSelection = 500 if INDEXNAME.lower() in 'banknifty' else 200
