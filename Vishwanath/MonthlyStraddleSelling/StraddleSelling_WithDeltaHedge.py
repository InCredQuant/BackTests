...
def deltaHedge(tradebookDf,deltaDf,greeksDf):
    sum_deltas = np.add(deltaDf[f'{tradetype}1Delta'], deltaDf[f'{tradetype}2Delta'])
    deltaDf['NetDelta'] = np.negative(sum_deltas) if tradetype == 'Short' else sum_deltas
    deltaDf['Flag'] = np.where(deltaDf['NetDelta'].abs() > deltaHedgeThreshold/100,1,0)
    deltaDf['Deltatohedge'] = np.where((deltaDf['Flag'] == 1) & (deltaDf['NetDelta']<0),deltaDf['NetDelta'] + deltaHedgeThreshold/100,
    np.where((deltaDf['Flag'] == 1) & (deltaDf['NetDelta']>0),deltaDf['NetDelta'] - deltaHedgeThreshold/100,np.nan))
    deltaDf['Deltatohedge'] = -deltaDf['Deltatohedge']
    deltaDf['Date'] = pd.to_datetime(deltaDf['Date'])
    greeksDf['Date'] = pd.to_datetime(greeksDf['Date'])
    deltaDf_filtered = deltaDf.dropna(subset=['Deltatohedge']).copy()
    deltaDf_filtered['MonthYear'] = deltaDf_filtered[f'{tradetype}1Ticker'].str.extract(r'(\d{1,2}[A-Z]{3}\d{2})')
    greeksDf['MonthYear'] = greeksDf['Ticker'].str.extract(r'(\d{1,2}[A-Z]{3}\d{2})')
    merged_all = pd.merge(
        deltaDf_filtered,
        greeksDf,
        on=['Date', 'MonthYear'],
    ...
    merged_all['DeltaDiff'] = (merged_all['Deltatohedge'] - merged_all['Delta']).abs()
    print(merged_all)
    xxxx
    closest_idx = merged_all.groupby(merged_all.index)['DeltaDiff'].idxmin()
    merged_final = merged_all.loc[closest_idx].drop(columns=['DeltaDiff'])
    print(merged_final)
    # deltaDf.to_excel("DeltaDfHedging.xlsx")

def fetchGreeksData():
    query = '''select "Date","Ticker","Delta"
                from greeks_nsefno
                where "Date" >= '2016-06-30'
                and "Ticker" like 'NIFTY%';'''
    connection = pg.connect(database="data", user="postgres", password="postgres", host="192.168.44.4", port=5432)
    greeks_data = pd.read_sql(query, connection)
    greeks_data['Date'] = pd.to_datetime(greeks_data['Date']).dt.date
    # greeks_data.to_csv("greeks_data.csv",index=False)
    return greeks_data
...
if __name__ == '__main__':
    ...
    # deltaCalculator()
    print("CALCULATING DELTA HEDGE")
    tradebook = pd.read_csv(rf"C:\Vishwanath\PythonCodes\Strategy\MonthlyStraddleSelling\NIFTY\Tradebook\Updated_23062025\Nifty_ShortStraddle_Monthly.csv")
    deltaDf = pd.read_excel(rf"C:\Vishwanath\PythonCodes\Strategy\MonthlyStraddleSelling\NIFTY\Delta\LongShort\Updated_23062025\Nifty_ShortStraddleDelta.xlsx")
    greeksData = pd.read_csv(rf"C:\Vishwanath\PythonCodes\Strategy\MonthlyStraddleSelling\greeks_data.csv")
    deltaHedge(tradebook.copy(),deltaDf.copy(),greeksData.copy())
