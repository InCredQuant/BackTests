...
# resamplFreq = 'w-FRI'#'2QS-NOV'#'w-FRI'#'A-JUN'#'w-FRI' #'2QS-APR'#'A-Mar'#
...
# mydata.indexpriceDaily = GetDataForIndicesFromBloomDB(priceconn, [indexName], 'PX_LAST')
...
# #DataStartDate = mydata.CloseDaily.index[0]
# mydata.CloseDaily = mydata.CloseDaily.loc[DataStartDate:]
# mydata.Close = mydata.CloseDaily.resample(resamplFreq, convention='end').last()
# mydata.Close = mydata.Close.loc[DataStartDate:]

# mydata.indexpriceDaily = mydata.indexpriceDaily.sort_index()
# mydata.indexpriceDaily = mydata.indexpriceDaily.loc[DataStartDate:]
...
# mydata.MarketCap = GetDataForTickersFromBloomDB(priceconn, mydata.allstocksinbloom, 'MCAP', DataStartDate)
# mydata.MarketCap = mydata.MarketCap/10 # Convertign Million to Cr
# mydata.MarketCap = mydata.MarketCap.resample(resamplFreq, convention = 'end').last()#mean()#mean

# ###################    ########################
# mydata.TurnOver = GetDataForTickersFromBloomDB(priceconn, mydata.allstocksinbloom, 'TURNOVER', DataStartDate)
# mydata.TurnOver = mydata.TurnOver.resample(resamplFreq, convention = 'end').last()
